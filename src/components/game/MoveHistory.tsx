  ...
  return (
    <Card className="flex-grow flex flex-col">
      ...
      <CardContent className="flex-grow overflow-hidden">
        <ScrollArea className="h-40 md:h-full w-full pr-4" viewportRef={scrollAreaRef}>
          <ol className="grid grid-cols-[auto_1fr_1fr] gap-x-4 gap-y-1 text-sm">
            {Array.from({ length: Math.ceil(moves.length / 2) }).map((_, i) => (
              <li key={i} className="contents animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
